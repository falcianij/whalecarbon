...
%% Initialization
load(strcat(wd, 'data_out\CTL.mat')); % lower resolution - less than a minute
whaleB_so_grid = load(strcat(wd, 'data\whaleB_gCm-2.csv'));
...
%%
test = flip(output.M3d(:,:,1));
writematrix(test, strcat(wd, 'data\water.csv'));
...
fg = whaleB_so_grid'; % transposed! % needs to be rectified s.t. it's deadfall (flux)
