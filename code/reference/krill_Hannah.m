...
D = readmatrix('krill_fpp_gCm-2yr-1.csv'); %import of dataset
...
surfacearea=grid.DXT3d(:,:,1).*grid.DYT3d(:,:,1).*M3d(:,:,1); %rows are latitude and columns are longitude
%matrix for southern ocean
southernoceanmatrix=zeros(91,180);
southernoceanmatrix(1:25,:)=1; 
matrix=southernoceanmatrix.*surfacearea; %we extract the southern ocean
%M3d covers if you hav eland or water (0 for land)
%but this is global
sumofrows=sum(matrix);
total_area=sum(sumofrows);
percentage= matrix/total_area;
biomass=2.6*percentage; %covers biomass for the southern ocean but dataset is as big as whole earth

% lato = [lato;latoe;latow]; %latitude of fecal production
% lono = [lono;lonoe;lonow]; %longitude of fecal production
% fo = [fo;foe;fow]; %amount of fecal production 
...
%% Martin curve for flux attenuation
z0 = 20; b = 0.5; flux = @(z) (z/z0).^(-b);
%If b is bigger then timeseq is smaller => flux is bigger so C stays less
%long in water
%If b is smaller, then seqtime is bigger => smaller flux so C stays longer
%in water
%You do minus b to get a positive flux
